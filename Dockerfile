...
RUN mkdir -p /tmp/grafana-src && \
    ...
    /tmp/mend update && \
    /tmp/mend dep -d "/tmp/grafana-src" -e -s "OpenScape UC Portfolio//OSEM third party applications//grafana-osem-sourcecode - $GRAFANA_VERSION-r$RELEASE_VERSION" -u --non-interactive && \
...
RUN mkdir -p /etc/grafana && \
    ...
    rpm -e --allmatches $(rpm -qa --qf "%{NAME}\n" | grep -v -E "bash|coreutils|filesystem|glibc$|libacl1|libattr1|libcap2|libgcc_s1|libgmp|libncurses|libpcre|libreadline|libselinux|libstdc\+\+|openSUSE-release|system-user-root|terminfo-base|sed") && \
    rm -Rf /etc/zypp && \
    rm -Rf /usr/lib/zypp* && \
